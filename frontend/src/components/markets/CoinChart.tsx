// components/markets/CoinChart.tsx
...
export default function CoinChart({ symbol, theme }: CoinChartProps) {
  ...
  const { candles, isLoading, isFetching, error, refetch } = useOhlc(
    symbol,
    timeframe,
    500,
  );

  // ── Compute chart colors from CSS vars ──
  const getChartColors = useCallback(() => {
    ...
    return {
      ...
      gridColor:
        style.getPropertyValue('--chart-grid').trim() || 'rgba(255,255,255,0.04)',
      crosshairColor:
        style.getPropertyValue('--chart-crosshair').trim() || '#6E7EA8',
